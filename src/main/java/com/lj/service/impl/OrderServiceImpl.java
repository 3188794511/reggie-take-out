...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Orders> implements OrderService {
    ...
    public void again(Orders orders) {
        //查询再来一单的订单基本信息
        Orders queryOrders = this.getById(orders.getId());
        //插入再来一单的基本信息(订单数据需要改变)
        long orderId = IdWorker.getId();//订单号
        queryOrders.setNumber(String.valueOf(orderId));
        queryOrders.setId(orderId);
        queryOrders.setOrderTime(LocalDateTime.now());
        queryOrders.setCheckoutTime(LocalDateTime.now());
        queryOrders.setStatus(2);
        this.save(queryOrders);
        //查询再来一单的所有详细信息
